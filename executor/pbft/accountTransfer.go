...
func (cphm *JZYPbftInsideExtraHandleMod) accountTransfer_do(atm *message.AccountTransferMsg) {
...
func (cphm *JZYPbftInsideExtraHandleMod) sendPartitionReady() {
	cphm.cdm.P_ReadyLock.Lock()
	...
	for sid := 0; sid < int(cphm.pbftNode.pbftChainConfig.ShardNums); sid++ {
		if sid != int(pr.FromShard) {
			go network.TcpDial(send_msg, cphm.pbftNode.ip_nodeTable[uint64(sid)][0])
		}
	}
...
func (cphm *JZYPbftInsideExtraHandleMod) getPartitionReady() bool {
	cphm.cdm.P_ReadyLock.Lock()
...
func (cphm *JZYPbftInsideExtraHandleMod) sendAccounts_and_Txs() {
	...
	for i := uint64(0); i < cphm.pbftNode.pbftChainConfig.ShardNums; i++ {
		...
		network.TcpDial(send_msg, cphm.pbftNode.ip_nodeTable[i][0])
...
// fetch collect infos
func (cphm *JZYPbftInsideExtraHandleMod) getCollectOver() bool {
	cphm.cdm.CollectLock.Lock()
...
func (cphm *JZYPbftInsideExtraHandleMod) proposePartition() (bool, *message.Request) {
	...
	for _, tx := range cphm.cdm.ReceivedNewTx {
		...
		if tx.Relayed && cphm.cdm.ModifiedMap[cphm.cdm.AccountTransferRound][tx.Receiver] != cphm.pbftNode.ShardID {
			log.Panic("error tx")
		}
	}
