...
func (bc *BlockChain) NewGenisisBlock() *core.Block {
	...
	blockheader.StateRoot = statusTrie.Hash().Bytes()
	blockheader.TxRoot = GetTxTreeRoot(body)
	b := core.NewBlock(blockheader, body)
...
func (bc *BlockChain) GetUpdateStatusTrie(txs []*core.Transaction) common.Hash {
	...
	for i, tx := range txs {
		...
		if !tx.Relayed && (bc.Get_PartitionMap(tx.Sender) == bc.ChainConfig.ShardID || tx.HasBroker) {
			...
			s_state_enc := st.Get([]byte(tx.Sender))
		...
		// recipientIn := false
		if bc.Get_PartitionMap(tx.Receiver) == bc.ChainConfig.ShardID || tx.HasBroker {
			// fmt.Printf("the recipient %s is in this shard %d, \n", tx.Recipient, bc.ChainConfig.ShardID)
			// recipientIn = true
			// modify local state
			r_state_enc := st.Get([]byte(tx.Receiver))
			...
			st.Update([]byte(tx.Receiver), r_state.Encode())
			cnt++
		}
...
func NewBlockChain(pcc *params.ChainConfig, db ethdb.Database) (*BlockChain, error) {
	fmt.Println("Generating a new blockchain", db)
	bc := &BlockChain{
		db:           db,
		ChainConfig:  pcc,
		Txpool:       core.NewTxPool(),
		Storage:      storage.NewStorage(pcc),
