...
type JZYPbftInsideExtraHandleMod struct {
	cdm      *construct.Construct_JZY
	pbftNode *PbftConsensusNode
}

// propose request with different types
func (cphm *JZYPbftInsideExtraHandleMod) HandleinPropose() (bool, *message.Request) {
...
func (cphm *JZYPbftInsideExtraHandleMod) HandleinPrePrepare(ppmsg *message.PrePrepare) bool {
...
func (cphm *JZYPbftInsideExtraHandleMod) HandleinPrepare(pmsg *message.Prepare) bool {
	fmt.Println("No operations are performed in Extra handle mod")
	return true
}

// the operation in commit.
func (cphm *JZYPbftInsideExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	if cphm.pbftNode.NodeID == cphm.pbftNode.view {
		...
		for _, tx := range block.Body {
			...
			rsid := cphm.pbftNode.CurChain.Get_PartitionMap(tx.Receiver)
		...
		for sid := uint64(0); sid < cphm.pbftNode.pbftChainConfig.ShardNums; sid++ {
			...
			go network.TcpDial(msg_send, cphm.pbftNode.ip_nodeTable[sid][0])
		...
		go network.TcpDial(msg_send, cphm.pbftNode.ip_nodeTable[params.LeaderShard][0])
...
func (cphm *JZYPbftInsideExtraHandleMod) HandleReqestforOldSeq(*message.RequestOldMessage) bool {
	fmt.Println("No operations are performed in Extra handle mod")
	return true
}

// the operation for sequential requests
func (cphm *JZYPbftInsideExtraHandleMod) HandleforSequentialRequest(som *message.SendOldMessage) bool {
