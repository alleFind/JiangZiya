...
func (p *PbftConsensusNode) handlePrePrepare(content []byte) {
	...
	if flag {
		...
		network.Broadcast(p.RunningNode.IPaddr, p.getNeighborNodes(), msg_send)
...
func (p *PbftConsensusNode) handleCommit(content []byte) {
	...
	if cnt >= required_cnt && !p.isReply[string(cmsg.Digest)] {
		...
		if _, ok := p.requestPool[string(cmsg.Digest)]; !ok {
			...
			network.TcpDial(msg_send, orequest.ServerNode.IPaddr)
		} else {
			// implement interface
...
func (p *PbftConsensusNode) handleRequestOldSeq(content []byte) {
	...
	network.TcpDial(msg_send, rom.SenderNode.IPaddr)
	...
	for {
		...
		case <-nextRoundBeginSignal:
			go func() {
				...
				go network.Broadcast(p.RunningNode.IPaddr, p.getNeighborNodes(), msg_send)
				p.pbftStage.Store(2)
			}()
